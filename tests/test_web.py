...
class WebTests(unittest.TestCase):
    ...
    def test_pages_include_printer_telemetry(self):
        ...
        self.assertNotIn('id="printer-chamber"', public_html)
        self.assertNotIn('id="printer-auxiliary-fan-2"', public_html)
        self.assertIn('id="printer-chamber"', admin_html)
        self.assertIn('id="printer-auxiliary-fan-2"', admin_html)
        self.assertIn("renderAmsStatus", public_script)
        self.assertIn("temperaturePair", public_script)
        self.assertIn("(Math.trunc(value * 10) / 10).toFixed(1)", public_script)
    ...
    def test_pages_show_active_and_queued_job_states(self):
        ...
        self.assertIn(".queue-page .queue-header {\n  display: grid;\n  grid-column: span 4;", styles)
        self.assertIn(".queue-page .current-print-card {\n  grid-column: span 8;", styles)
        self.assertIn(".queue-page .header-metrics {\n  grid-template-columns: repeat(2, minmax(0, 1fr));", styles)
        self.assertIn(".queue-page .queue-card {\n  grid-column: span 7;", styles)
        self.assertIn(".queue-page .materials-submit-card {\n  grid-column: span 5;", styles)
        self.assertIn(".queue-page .ams-status {\n  grid-template-columns: repeat(2, minmax(0, 1fr));", styles)
        ...
        self.assertIn('id="printer-alert" role="alert" hidden', admin_html)
        self.assertIn("styles.css?v=printer-errors-1", admin_html)
        self.assertIn("admin.js?v=printer-errors-1", admin_html)
        self.assertIn("renderPrinterAlert", admin_script)
        self.assertIn("debug?.printer?.active_error", admin_script)
